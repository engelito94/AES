...
String filePath = GlobalVariable.cestaSubor + 'Y-04 SK515 - súhrnné CV_TYP_Y_VZOR.xml'

String filePathDialog = filePath.replaceAll('/', '\\\\')

xml.replaceLrnValue(filePath, LRN)
xml.updateY(filePath)
...
Windows.verifyElementPresent(findWindowsObject('Object Repository/CIS_klient/CU_vyvozu/SuhrnneCVTypuY'), 0)
...
if (!(upozornenia.contains('Súhrnné colné vyhlásenie.'))) {
    log.logFailed('Operácie v hornej časti záznamu nie sú podľa očakávania. ' + 'Upozornenia : ' + upozornenia )
	util.markErrorAndStop("Operácie v hornej časti záznamu nie sú podľa očakávania.")
}

Windows.click(findWindowsObject('Object Repository/CIS_klient/CU_vyvozu/VyhladaneCV/TabItem_CiastkoveCV'))

if(Windows.verifyElementPresent(findWindowsObject('Object Repository/CIS_klient/CU_vyvozu/VyhladaneCV/TableItem_SuhrnneCV1'), 0) 
	&& Windows.verifyElementPresent(findWindowsObject('Object Repository/CIS_klient/CU_vyvozu/VyhladaneCV/TableItem_SuhrnneCV1'), 0)) 
{
	log.logPassed("Dve čiastkové CV sú zobrazené")	
} else {
	util.markFailedAndStop("Nie sú zobrazené 2 čiastkové CV")
}
